...
- **ADB 无线配对** — 支持 Android 11+ 无线调试配对（`adb pair`）
- **设备连接自动运行任务** — 配置自动任务列表，新设备连接时自动入队并启动执行
...
- **Docker 支持** — 多阶段构建，GitHub Actions 自动打包推送
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/tasks/batch-enqueue` | 将同一个脚本批量分配到多台设备 |
| `POST` | `/api/tasks/batch-start` | 批量启动设备队列 |
| `POST` | `/api/tasks/batch-stop` | 批量停止设备队列 |

### 自动任务设置

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/settings/auto-tasks` | 获取自动运行任务脚本列表 |
| `PUT` | `/api/settings/auto-tasks` | 设置自动运行任务脚本列表（设备连接时自动执行） |

### ADB 配对

| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/devices/pair` | ADB 无线调试配对（Android 11+） |

**配对请求示例：**

```json
{
  "address": "192.168.1.100:41339",
  "code": "123456"
}
```
